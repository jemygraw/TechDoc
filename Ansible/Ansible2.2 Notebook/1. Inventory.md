...
13. Ansible 中的两个默认分组 `all` 和 `ungrouped`，其中`all`表示所有的Host，`ungrouped`表示所有不在Group中的Host；
14. 基于上面的默认分组的定义，任何Host都至少在两个分组中。这个结论是这样推导的，首先一个Host必须在`all`这个分组中，其次，如果这个Host不再任何其他分组中，则必然在`ungrouped`分组中；
15. 我们一般建议将 Host，Group 的定义和它们的属性分开，即将目标主机的属性从 Inventory 文件中拆分开来；
16. 拆分出来的目标Host和Group的属性通过 YAML 格式的文件来存储，下面的几个文件分别定义了不同Host和Group的属性；
```
# Inventory File
/etc/ansible/hosts
...
21. 从 Ansible2.4 开始，支持Group变量 `ansible_group_priority` 来改变相同层级的 Grop 的合并优先级，priority越高，会放在后面合并，默认都为1；
