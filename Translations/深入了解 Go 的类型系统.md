...
现在来仔细看下这几个代码片段，研究下哪个片段可以成功编译，另外无论代码是否编译可以解释下原因。
...
预定义的类型如 int，int64，float32，string 和 bool 都成为命名类型。即所有的预定义的布尔型，数值类型和字符串类型都是命名类型。
...
1. 值 x 的类型和 T 一致；
2. 值 x 的类型 v 和类型 T 的底层类型一致，并且 v 和 T 中至少有一个不是自定义的类型；
3. T 是一个接口类型，并且值 x 的类型实现了该接口类型 T；
4. 值 x 的类型是一个双向的通道类型，T 是一个通道类型，值 x 的类型 v 和 T 拥有相同的元素类型，并且 v 和 T 中至少有一个不是自定义的类型；
...
**原因：** 代码中的 i 是一个命令类型 int，而 ai 则是另外一个命名类型 aInt，虽然它们的底层类型都是 int。
...
上面的代码可以编译，因为 m 是一个未命名的类型，同时 m 和 mMap 的底层类型是一致的。
...
3. 忽略结构体的标签，值 x 的类型和 T 都是非自定义类型的指针类型，并且它们的指针类型的基本类型拥有相同的底层类型；
...
名称一致性最直接，两个类型当且仅当在名称相同的时候是一致的。在 Go 里面一个自定义的类型总是和其他的类型不同就是基于这个原则。
